...
\begin{proposition}\label{equivrel-to-decomp}
    \uses{decomposition-def}
    \lean{equivrel_to_decomp}
    \leanok
...
\begin{proposition}
    \label{decomp-equiv}
    \uses{left-function-right-action, symmetric}
...
\begin{proposition}
    ...
    \uses{decomp-action, decomp-equiv}
    $X$が左$G$作用を持つとする。
...
\begin{proposition}\label{subgroup-action}
    \uses{subgroup-def,left-action-def}
    集合$X$に群$G$が左作用するとき、
...
\begin{proposition}\label{group-decomp}
    \uses{decomp-equiv-action, subgroup-def,decomposition-def,left-action-def,subgroup-decomp,left-fixed-point,decomp-action,decomp-equiv,left-multiplication,powerset-action}
